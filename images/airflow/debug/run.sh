...
download_airflow_constraints() {
  local airflow_version="$1"
  local python_version="$2"

  if [[ -z "$airflow_version" || -z "$python_version" ]]; then
    echo "Usage: download_airflow_constraints <airflow_version> <python_version>"
    return 1
  fi

  local truncated_python_version=$(echo $python_version | cut -d '.' -f 1-2)
  local base_url="https://raw.githubusercontent.com/apache/airflow"
  local constraints_url="$base_url/constraints-${airflow_version}/constraints-${truncated_python_version}.txt"
  local output_dir="./etc"
  local output_file="${output_dir}/airflow_constraints.txt"

  mkdir -p "$output_dir"

  echo "Downloading constraints from: $constraints_url"
  if curl -fSL "$constraints_url" -o "$output_file"; then
    echo "Constraints file saved to: $output_file"
  else
    echo "❌ Failed to download constraints file. Check that the Airflow and Python versions are valid."
    return 2
  fi
}
download_airflow_constraints $AIRFLOW_VERSION $PYTHON_VERSION

# Build the Docker image
./build.sh $CONTAINER_RUNTIME $AIRFLOW_VERSION $PYTHON_VERSION
